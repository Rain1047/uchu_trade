...
埋头开发一个月了，借用了Claude的能力，整个的系统已经初具雏形。已经具备了历史日志、现货根据配置动态止损、限价购入；

开发的工作可以说迈入正轨，后续还需要进行优化和完善。下面是需要注意的几个地方：

1. 回测系统 - 作为所有策略好坏评价的核心，回测系统的产品化、特别是从操作的便捷性、和可视化结果的展示，都需要相应的产品设计和代码开发。
2. 策略实例链路 - 也是后续的主要链路，交易系统的核心还是针对合约类的交易，主链路的设计还需要进一步思考。对于主链路部分，已经加入了责任链、工厂等模式，一定程度上提高了系统的可扩展性。虽然后续还需要进一步的细节探究，但是有比没有好，跑通比做好更重要；
3. 调度链路可视化 - 由于策略的实例是通过调度任务来实现的，因此需要有一个可视化的调度链路来展示调度任务的执行情况。目前的调度链路是通过日志来展示的，但是这种方式不够直观，需要有一个可视化的展示方式。
4. 系统监控/重试/异常处理 - 系统的监控、重试、异常处理是系统的稳定性支撑，目前还没有考虑到
5. 交易明细可视化 - 后续必备，总得知道亏了多少钱嘛哈哈哈～

回看了开始7月份写的计划，【后续想法：对于调度任务，可以根据职能划分为：买入、止损、离场三个调度任务，后续可以捕捉热点的调度任务（和纯量化有点冲突，但是对于系统来说可以考虑设计）】

感觉虽然整体的大方向没有变化，但是在具体实现上还是有非常大的进步。用时间间隔作为调度任务的区别，将时间间隔、买入、离场、止损作为可配置化的条件放到策略实例上；

以及对于调度任务健壮性、可扩展性的探索，使得整体的实现优雅了很多。自己之前对于设计模式其实还是有点摸不着头脑的，但是现在至少可以针对具体的场景想到相应的设计模式了，也算是个提升～

后续继续埋头开发一阵子，代码也闭源一阵子吧～

## 2024-11-28

今天，终于，开发完成了回测的系统的后端部分，太不容易了。中间真的是，踩了无数无数次坑，解决问题还是需要回归本质，后续的路就好走了，现在离可视化就是临门一脚。

## 2024-12-08

截止今天，回测系统的可视化部分已经完成了百分之50%. 已经有了完整的一个可视化的界面，
但是在和后端交互上还是没有完成，计划在后续和完整交易链路的开发工作同步进行；

策略系统和交易系统是紧密关联的，中间还需要具体更加精细化的设计：除了最基本的交互之外，还需要考虑能否将回测结果的优劣作为是否创建实例的依据或者信号；

对于完整交易链路的后端部分开发也初步搭建，再次遇到了一些问题，其中最关键的时不了解okx对于实体和实例之间的耦合关系，踩了不少的坑；
但是好在一点一点的磨出来了，这里自己还是需要反思下的，作为一个开发，应该有一定的工程角度思考问题的能力，当然有的时候就是思维的盲区，
这个盲区指的是第一时间思考问题的角度，这里的确还需要转换。


## 2024-12-11

最近情绪大多消耗在了盯盘上，有空闲的时间都用来盯盘了，这样不好。

系统的开发也开始遇到了些瓶颈，今天先是环境都挂了，重装了一遍，还好之前有记录安装环境里面的坑。

总之希望放松下来调整调整自己的脑子，回回血，再继续开发。特别是主链路这里，急不得。

只有宏观的目的却没有详细的目标是很不好一件事，整个链路的开发太笼统了，一下子失去了方向感，这两天先把前面的工作盘点下，后面的任务梳理下。

## 2024-12-12

今天回来整理了下代码和头绪，发现调度的完整链路已经跑通了。虽然还有很多的细节需要打磨，整体也算一件好事。开发的感觉没有完全找回来，感觉说句中肯的话
房间里面太冷了，这南方的天怎么这么冷，家里这时候暖气烧的已经短袖+裤衩子了，打算新买一个暖气，贵点就贵点，腿一冷根本坐不住的。进被窝了先～